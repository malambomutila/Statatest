...
statatest assert_equal r(min) 1990   // actual = r(min), expected = 1990
//                     ↑      ↑
//                  actual  expected
```
...
```stata
summarize mpg, meanonly
statatest assert_equal r(N) 74,   msg("summarize counted 74 observations")
...
```stata
summarize mpg, meanonly
statatest assert_approx r(mean) 21.2973, tol(0.001) msg("mean mpg is about 21.30")
```
...
statatest begin "mycenter -- basic behaviour"
  ...
  statatest assert_approx r(mean) 0, tol(1e-8) msg("mean of centred variable is 0")
  ...
  statatest assert (r(min) < 0), msg("some values are below the mean")
  statatest assert (r(max) > 0), msg("some values are above the mean")
...
statatest begin "Suite 5: Epidemiological consistency"
    ...
    statatest assert (`afr_mean' > r(mean)), msg("Africa mean incidence > Europe mean incidence")
statatest end
```
